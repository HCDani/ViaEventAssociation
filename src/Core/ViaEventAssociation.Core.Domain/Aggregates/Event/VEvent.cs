...
namespace ViaEventAssociation.Core.Domain.Aggregates.EventNS {
    public class VEvent : AggregateRoot<Guid> {
        ...
        public Result<Location> UpdateLocation(Location location) {
            ...
            if (Status == Status.Cancelled) return new Result<Location>(62, "Location cannot be updated when status is cancelled.");
            if (Status == Status.Active) return new Result<Location>(63, "Location cannot be set to private when status is active.");
            Location = location;
